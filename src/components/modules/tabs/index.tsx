    ...
    <div className="tabs" ref={ref} role="tablist" aria-label="Experience Tabs">
      {props.tabs.map((tab, index) => (
        <button
          ...
          role="tab"
          className={clsx(
            'py-1.5 pt-2 small-copy text-center hover:text-primary-foreground hover:bg-primary motion-safe:transition-all motion-safe:duration-300 motion-safe:ease-in-out',
            {
              'bg-primary text-primary-foreground': index === props.selectedTab,
            },
            'sm:py-2.5 sm:px-8',
            props.className,
          )}
      ...
      <div
        className="
          hidden w-px border border-solid border-primary-foreground absolute z-20 right-0 motion-safe:transition-top motion-safe:ease-in-out motion-safe:duration-300
          sm:block
        "
